...
```go
import (
	"github.com/stainless-sdks/context.dev-go" // imported as contextdev
)
```
...
See the [full list of request options](https://pkg.go.dev/github.com/stainless-sdks/context.dev-go/option).
...
`*contextdev.Error`. This contains the `StatusCode`, `*http.Request`, and
...
We also provide a helper `contextdev.File(reader io.Reader, filename string, contentType string)`
